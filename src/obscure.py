...
class PartialOcclusionGenerator:
        ...
        for img_id, img_info in self.images.items():
            ...
            if occlusion_type == "crop":
                obscured_image, updated_annotations = self._apply_image_crop(
                    image, self.image_annotations[img_id], occlusion_ratio, img_id
                )
    ...
    def _apply_occlusion(
        ...
        if occlusion_type == "patch":
            ...
            noisy_region = np.clip(
                image[y1:y2, x1:x2].astype(np.float32) + noise, 0, 255
            ).astype(np.uint8)
...
def main():
    ...
    parser.add_argument(
        "--dataset", type=str, required=True, help="Path to input dataset directory"
    )
    parser.add_argument(
        "--output",
        type=str,
