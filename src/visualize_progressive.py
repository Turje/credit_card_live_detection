...
def visualize_progressive_occlusion(
    ...
    occlusion_datasets: dict[int, str | Path],  # {25: path, 50: path, 75: path}
    output_path: str | Path = "outputs/progressive_occlusion_comparison.png",
    ...
    if image_name:
        orig_img_info = next(
            (img for img in orig_data["images"] if img["file_name"] == image_name), None
        )
    ...
    plt.suptitle(
        f"Progressive Occlusion: {img_filename}", fontsize=16, fontweight="bold", y=1.02
    )
    plt.tight_layout()
    ...
    print(f"✅ Progressive occlusion visualization saved to: {output_path_obj}")


def main():
    parser = argparse.ArgumentParser(
        description="Visualize progressive occlusion on single image"
    )
    parser.add_argument(
        "--original", type=str, required=True, help="Path to original dataset"
    )
    parser.add_argument(
        "--occlusion-25", type=str, required=True, help="Path to 25% occlusion dataset"
    )
    parser.add_argument(
        "--occlusion-50", type=str, required=True, help="Path to 50% occlusion dataset"
    )
    parser.add_argument(
        "--occlusion-75", type=str, required=True, help="Path to 75% occlusion dataset"
    )
    parser.add_argument(
        "--image",
        type=str,
