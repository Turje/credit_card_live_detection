...
def prepare_dataset_config(
    dataset_path: str | Path, output_config: str | Path = "dataset.yaml"
) -> str:
    """
    ...
    config = {
        "path": (
            str(split_base.absolute())
            if split_base.exists()
            else str(dataset_path.absolute())
        ),
        "train": f"{train_path}",
        "val": f"{val_path}",
    ...
    print(f"Dataset config created: {output_config_path}")
    print(f"Classes: {class_names}")

    return str(output_config_path)


def convert_coco_to_yolo(dataset_path: str | Path) -> None:
    """
...
def train_model(
    ...
    split_base = (
        dataset_parent / f"{Path(dataset_path).name.replace('_split', '')}_split"
    )
    ...
    if split_base.exists():
        ...
        val_path = (
            ...
            if (Path(dataset_path) / "val").exists()
            else train_path
        )
    ...
    if not yolo_labels.exists() or len(list(yolo_labels.glob("*.txt"))) == 0:
        ...
        convert_coco_to_yolo(
            str(train_path.parent) if split_base.exists() else dataset_path
        )
        # Convert val set if separate
        if val_path != train_path and val_path.exists():
            convert_coco_to_yolo(
                str(val_path.parent) if split_base.exists() else str(val_path.parent)
            )
