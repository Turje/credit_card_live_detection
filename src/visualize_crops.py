...
def visualize_crops(
    ...
    orig_anns: Dict[int, List[Dict[str, Any]]] = {
        img_id: [] for img_id in orig_images.keys()
    }
    for ann in orig_data["annotations"]:
        orig_anns[ann["image_id"]].append(ann)

    occ_images = {img["id"]: img for img in occ_data["images"]}
    occ_anns: Dict[int, List[Dict[str, Any]]] = {
        img_id: [] for img_id in occ_images.keys()
    }
    ...
    for img_name, img_info in orig_img_files.items():
        ...
        occ_img = next(
            (img for img in occ_data["images"] if img["file_name"] == img_name), None
        )
    ...
    # Add title
    fig.suptitle(
        "Cropped Object Regions: Original vs Occluded",
        fontsize=16,
        fontweight="bold",
        y=0.98,
    )
    ...
    print(f"✅ Crop visualization saved to: {output_path_obj}")
...
def main():
    ...
    parser.add_argument(
        "--occluded", type=str, required=True, help="Path to occluded dataset"
    )
    parser.add_argument(
        "--samples", type=int, default=6, help="Number of samples to show (default: 6)"
    )
    parser.add_argument(
        "--output",
        type=str,
