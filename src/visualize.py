...
class DatasetVisualizer:
    ...
    def create_overlay_images(self, output_dir: Optional[str | Path] = None) -> None:
        """
        ...
        if output_dir is None:
            output_dir_obj = self.dataset_path / "overlay"
        else:
        ...
        for img_id, img_info in self.images.items():
            ...
            # Save overlay image
            output_path = output_dir_obj / img_filename
            image_bgr = cv2.cvtColor(image_rgb, cv2.COLOR_RGB2BGR)
            cv2.imwrite(str(output_path), image_bgr)

        print(f"Created {len(self.images)} overlay images in {output_dir_obj}")

    def generate_statistics(self, output_dir: Optional[str | Path] = None) -> None:
        """
        ...
        """
        if output_dir is None:
            output_dir_obj = Path("outputs") / "stats"
        else:
            output_dir_obj = Path(output_dir)
        ...
        self._plot_class_distribution(stats, output_dir_obj)
        self._plot_bbox_statistics(stats, output_dir_obj)
        self._plot_bbox_size_distribution(stats, output_dir_obj)

        # Print summary
        self._print_summary(stats)

        print(f"\nStatistics plots saved to {output_dir_obj}")
    ...
    def _plot_class_distribution(self, stats: Dict[str, Any], output_dir: Path) -> None:
        ...
        colors_list = cmap(np.linspace(0, 1, len(classes)))
        ...
        # Pie chart
        ax2.pie(
            counts, labels=classes, autopct="%1.1f%%", colors=colors_list, startangle=90
        )
    ...
    def _plot_bbox_statistics(self, stats: Dict[str, Any], output_dir: Path) -> None:
        ...
        axes[0, 0].set_title(
            f"Average Bounding Box Size\nWidth: {avg_width:.1f}px, Height: {avg_height:.1f}px"
        )
        axes[0, 0].grid(axis="y", alpha=0.3)

        # Width distribution
        axes[0, 1].hist(
            stats["bbox_widths"], bins=30, color="skyblue", edgecolor="black", alpha=0.7
        )
        axes[0, 1].axvline(
            avg_width,
            color="red",
        ...
        axes[1, 1].hist(
            ...
            bins=range(0, max(stats["annotations_per_image"]) + 2),
            color="lightgreen",
            edgecolor="black",
            alpha=0.7,
        )
    ...
    def _plot_bbox_size_distribution(
        ...
        ax.scatter(
            ...
            marker="x",
            linewidth=3,
            label=f"Average ({avg_width:.1f}, {avg_height:.1f})",
        )
        ...
        plt.tight_layout()
        plt.savefig(
            output_dir / "bbox_size_distribution.png", dpi=150, bbox_inches="tight"
        )
        plt.close()
    ...
    def _print_summary(self, stats: Dict[str, Any]) -> None:
        ...
        print(
            f"Average Annotations per Image: {np.mean(stats['annotations_per_image']):.2f}"
        )
        print("\nClass Distribution:")
    ...
    def _generate_colors(self, n):
        ...
        return [
            (int(r * 255), int(g * 255), int(b * 255)) for r, g, b, _ in colors_list
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Visualize dataset and create overlays"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--stats", action="store_true", help="Generate statistics and visualizations"
    )
    parser.add_argument(
        "--overlay-dir",
        type=str,
