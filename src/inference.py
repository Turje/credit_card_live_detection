...
def main():
    ...
    # Output
    parser.add_argument("--output", type=str, help="Output path (video or image)")
    ...
    parser.add_argument(
        "--show", action="store_true", help="Show video during processing"
    )
    ...
    try:
        ...
        if framework == "ultralytics":
            inference = UltralyticsInference(
                model_path=args.model, device=config.inference.device
            )
            ...
            if args.video:
                ...
                if not output_path.endswith(".mp4"):
                    output_path = (
                        str(Path(output_path) / Path(args.video).stem) + "_detected.mp4"
                    )
                ...
                # Process image
                output_path = config.inference.output_path
                if Path(output_path).is_dir() or not output_path.endswith(
                    (".jpg", ".png", ".jpeg")
                ):
