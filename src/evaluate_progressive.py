...
def evaluate_model_on_test_set(
    model_path: str | Path, test_dataset_path: str | Path
) -> Dict[str, float]:
    """
    ...
    existing_images = list(yolo_images_dir.glob("*.jpg")) + list(
        yolo_images_dir.glob("*.png")
    )
        ...
        for img_id, img_info in images.items():
            ...
            with open(label_file, "w") as f:
                for ann in img_anns:
                    ...
                    f.write(
                        f"{yolo_cat_id} {center_x:.6f} {center_y:.6f} {norm_w:.6f} {norm_h:.6f}\n"
                    )
        print(f"  ✅ Created {len(images)} label files and copied images")
    else:
        print(
            f"  ✅ YOLO format already exists ({len(existing_labels)} labels, {len(existing_images)} images)"
        )

    # Verify images and labels exist
    image_files = list(yolo_images_dir.glob("*.jpg")) + list(
        yolo_images_dir.glob("*.png")
    )
...
def evaluate_progressive(
    model_path: str | Path,
    test_sets_base: str | Path,
    output_dir: str | Path = "outputs/progressive_evaluation",
    ...
    results_file = output_dir_obj / "progressive_results.json"
    ...
    plot_progressive_results(results, output_dir_obj)

    return results


def plot_progressive_results(
    results: Dict[int, Dict[str, float]], output_dir: Path
) -> None:
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate model on progressive occlusion test sets"
    )
    parser.add_argument(
    ...
    try:
        evaluate_progressive(
            model_path=args.model, test_sets_base=args.test_sets, output_dir=args.output
        )
    except Exception as e:
        print(f"Error: {e}")
        import traceback
