...
class TestUltralyticsTrainer(unittest.TestCase):
    ...
    def test_train_basic(self, mock_prepare_config, mock_convert, mock_yolo):
        ...
        results = trainer.train(
            ...
            img_size=640,
            batch_size=8,
            output_dir=str(self.temp_dir / "models"),
            device="cpu"
        )
    ...
    def test_device_mapping(self, mock_yolo):
        ...
        for device_input, device_expected in [("cuda", "0"), ("cpu", "cpu"), ("mps", "mps")]:
            with patch('src.models.ultralytics_trainer.convert_coco_to_yolo'), \
                 patch('src.models.ultralytics_trainer.prepare_dataset_config', return_value="dataset.yaml"):
                train_dir = self.dataset_path / "train"
                (train_dir / "labels").mkdir()
